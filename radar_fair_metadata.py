...
class RadarFairMetadata(object):
    ...
    def __init__(self):
        # type: () -> object
        self.radar_api = RadarApi(
            host='http://radar-test.thehyve.net/',
            user='admin',
            password='changeme')
    ...
    def repository(self):
        ...
        return self._triples_to_graph([
            (repository, self.DCT.title, Literal('RADAR')),
            (repository, self.FDP.metadataIssued, Literal(datetime(2017, 2, 17))),
            (repository, self.DCT.hasVersion, Literal(1)),
            (repository, self.DCT.description,
             Literal('This is the development instance of RADAR FAIR Datapoint.', lang='en')),
    ...
    def catalog(self):
        ...
        studies_result = self.radar_api.get_studies()
    ...
    def dataset(self, study_id):
        study = self.radar_api.get_studies(study_id)
        dataset = URIRef('')
